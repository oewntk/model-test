...
object LibModelEquals {

    const val FAIL = true

    fun testZipLexes(lexes1: Set<Lex>, lexes2: Set<Lex>) {
        lexes1.zip(lexes2).forEach { (lex1, lex2) ->
            if (lex1 != lex2) {
                ...
                if (FAIL) fail(report) else Tracing.psErr.println(report)
            }
        }
        assertTrue(lexes1 == lexes2)
    }

    fun testZipSenses(senses1: Set<Sense>, senses2: Set<Sense>) {
    ...
    fun testZipSynsets(synsets1: Set<Synset>, synsets2: Set<Synset>) {
